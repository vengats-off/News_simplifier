...
# Configuration
class Config:
    # NewsAPI key - Get from environment variable or use default
    ...
    def fetch_news_from_newsapi(self, query, language='en', sort_by='publishedAt', page_size=20):
        ...
        try:
            if self.newsapi_key == 'your_newsapi_key_here':
                logger.warning("NewsAPI key not configured, using web scraping fallback")
                return []
...
def health_check():
    ...
    return jsonify({'status': 'healthy', 'service': 'SagiX News Simplifier'}), 200

if __name__ == '__main__':
    port = int(os.environ.get('PORT', 5000))
    app.run(host='0.0.0.0', port=port, debug=False)from flask import Flask, render_template, request, jsonify
from flask_cors import CORS
import requests
import re
import time
